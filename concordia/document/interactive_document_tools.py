...
class InteractiveDocumentWithTools(interactive_document.InteractiveDocument):
  ...
  def _run_policy(
    ...
    try:
      ...
      note = self._format_policy_note(
          action='error_enforced', reason=str(error)
      )
      result = (
          f'Error: Tool call "{tool_name}" blocked due to policy error.'
      )
    ...
    if action is tool_policy.PolicyAction.ALLOW:
    ...
    if action is tool_policy.PolicyAction.DENY:
      if self._enforcement_mode == 'observe':
    ...
    if action is tool_policy.PolicyAction.EDIT:
      if self._enforcement_mode == 'observe':
      ...
      if not isinstance(decision.edited_args, dict):
        merged_tags = self._merge_tags((TOOL_POLICY_DENY_ENFORCED_TAG,), tags)
        note = self._format_policy_note(
            action='deny_enforced',
            reason='Policy requested edit without edited_args.',
        )
        result = f'Error: Tool call "{tool_name}" denied by policy.'
        return args, result, merged_tags, note
    ...
    merged_tags = self._merge_tags((TOOL_POLICY_DENY_ENFORCED_TAG,), tags)
    note = self._format_policy_note(
        action='deny_enforced', reason='Unknown policy action.'
    )
    result = f'Error: Tool call "{tool_name}" denied by policy.'
    return args, result, merged_tags, note
